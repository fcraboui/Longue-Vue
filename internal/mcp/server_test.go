...
func (p *pagingFakeStore) ListClusters(_ context.Context, _ api.ClusterListFilter, _ api.ListPage) ([]api.Cluster, string, error) {
...
func TestCollectAll_TruncatesAtMaxTotalItems(t *testing.T) {
	...
	got, err := collectAll(context.Background(), func(ctx context.Context, cursor string) ([]api.Cluster, string, error) {
		return p.ListClusters(ctx, api.ClusterListFilter{}, api.ListPage{Limit: maxPageSize, Cursor: cursor})
	})
	if err != nil {
