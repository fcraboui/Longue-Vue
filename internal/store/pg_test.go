...
func newTestPG(t *testing.T) *PG {
	...
	t.Cleanup(func() {
		...
		_, _ = pg.pool.Exec(context.Background(),
			"TRUNCATE applications, application_blocks, clusters, cloud_accounts, "+
				"image_versions, image_origin_resolutions, image_origin_mappings, "+
...
func TestSettings_ImageVersionsEnabled_RoundTrip(t *testing.T) {
	...
	// Reset image_versions_enabled to false on test exit so subsequent
	...
	t.Cleanup(func() {
		_, _ = pg.pool.Exec(context.Background(),
			"UPDATE settings SET image_versions_enabled = FALSE WHERE id = 1")
	})
